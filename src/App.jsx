...
import { getFirestore, doc, setDoc, deleteDoc, onSnapshot, collection, addDoc, query, orderBy, limit } from "firebase/firestore";

// ⚠️ ใช้ Firebase Config ของคุณ
...
const Icons = {
  Plus: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4"/></svg>,
  Search: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>,
  Edit: ({ c = "" }) => <svg className={`w-4 h-4 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>,
  Trash: ({ c = "" }) => <svg className={`w-4 h-4 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>,
  Package: ({ c = "" }) => <svg className={`w-6 h-6 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>,
  Alert: ({ c = "" }) => <svg className={`w-12 h-12 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>,
  Settings: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/></svg>,
  X: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/></svg>,
  Tag: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/></svg>,
  History: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>,
  UserPlus: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"/></svg>,
  CheckCircle: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>,
  Unlock: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 11V7a4 4 0 118 0m-4 8v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2z"/></svg>,
  Lock: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>,
  Download: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>,
  Upload: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>,
  ClipboardList: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"/></svg>,
  Folder: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"/></svg>,
  ViewGrid: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"/></svg>,
  Camera: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 13a3 3 0 11-6 0 3 3 0 016 0z"/></svg>,
  VideoCamera: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>,
  Speaker: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z"/></svg>,
  Users: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/></svg>,
  Signal: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.111 16.404a5.5 5.5 0 017.778 0M12 20h.01m-7.08-7.071c3.904-3.905 10.236-3.905 14.141 0M1.394 9.393c5.857-5.857 15.355-5.857 21.213 0"/></svg>,
  Eye: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>,
  EyeOff: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"/></svg>,
  Sun: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"/></svg>,
  Moon: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"/></svg>,
  Layers: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 002-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>,
  Monitor: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>,
  Truck: ({ c = "" }) => <svg className={`w-5 h-5 ${c}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M10 17h4V5H2v12h3"/><path d="M20 17h2v-3.34a4 4 0 0 0-1.17-2.92l-1.09-1.09A4 4 0 0 0 16.92 9H14v8h2"/><circle cx="8.5" cy="17.5" r="2.5"/><circle cx="18.5" cy="17.5" r="2.5"/></svg>,
  QrCode: ({ c = "" }) => <svg className={c} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v1m-4 8h.01M16 12h.01M8 16h.01M16 16h.01M4 4h6v6H4V4zm10 0h6v6h-6V4zM4 14h6v6H4v-6z"/></svg>,
  Printer: ({ c = "" }) => <svg className={c} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg>
};
...
function MainApp() {
  ...
  useEffect(() => { if (showScanModal && scanInputRef.current) { scanInputRef.current.focus(); } }, [showScanModal, useCamera]);
  ...
  useEffect(() => {
    ...
    if (showAuditModal || showCommandCenter) {
      const q = query(getAuditCol(), orderBy('timestamp', 'desc'), limit(auditLimit));
  ...
  }, [user, showAuditModal, showCommandCenter, auditLimit]);
  ...
  const handleBorrow = async () => {
    ...
    try {
      const promises = packingChecklist.map(id => {
        ...
        const newHistory = [...(item.history || []), newHistoryEntry].slice(-100); 
  ...
  const exportToCSV = () => {
    ...
    const csvData = items.map(i => {
      ...
      const historyStr = (i.history || []).map(h => {
        ...
        if(h.type === 'event') return `[${d}] งาน:${h.eventName}`;
      ...
      return [ 
        ...
        i.quantity || 1, i.currentBorrower || i.currentEvent || '-', i.owner || '-', 
        new Date(i.updatedAt).toLocaleDateString('th-TH'),
    ...
    return (
      <div className="bg-white min-h-screen font-sans text-black">
         <div className="print:hidden p-4 bg-slate-800 text-white flex justify-between items-center fixed top-0 w-full z-50 shadow-md">
            <h2 className="font-bold text-xl flex items-center gap-2"><Icons.QrCode c="w-6 h-6"/> พิมพ์ QR Code ({selectedItems.length} ดวง)</h2>
            <div className="flex gap-3"><button onClick={() => window.print()} className="bg-blue-600 hover:bg-blue-500 px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 transition-colors"><Icons.Printer c="w-5 h-5"/> สั่งพิมพ์</button><button onClick={() => setShowPrintModal(false)} className="bg-slate-600 hover:bg-slate-500 px-6 py-2.5 rounded-xl font-bold transition-colors">ปิด</button></div>
         </div>
      ...
      <div className={`fixed inset-0 font-sans z-[10000] flex flex-col p-4 sm:p-8 overflow-hidden font-medium transition-colors duration-300 ${isDarkMode ? 'bg-slate-950 text-slate-200' : 'bg-slate-50 text-slate-800'}`}>
        <div className={`flex flex-col sm:flex-row justify-between items-center mb-6 p-4 sm:px-8 sm:py-5 rounded-3xl shadow-sm border gap-4 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
          <h1 className={`text-2xl sm:text-3xl font-black tracking-tight flex items-center gap-3 ${isDarkMode ? 'text-indigo-400' : 'text-indigo-600'}`}>
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-inner ${isDarkMode ? 'bg-indigo-900/50' : 'bg-indigo-100'}`}><Icons.Monitor c="w-7 h-7"/></div> ศูนย์ควบคุม MDEC ✨
          </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 min-h-0">
          <div className="flex flex-col gap-6">
            <div className={`p-8 rounded-3xl flex flex-col items-center justify-center relative overflow-hidden shadow-lg ${isDarkMode ? 'bg-gradient-to-br from-blue-900/80 to-indigo-900/80' : 'bg-gradient-to-br from-blue-400 to-indigo-500'}`}>
              <h2 className="text-xl font-bold mb-2 z-10 flex items-center gap-2 text-white"><Icons.Package c="w-6 h-6"/> อุปกรณ์ทั้งหมด</h2>
          ...
          <div className="flex flex-col gap-6">
            ...
            {activeOutItems.length > 0 ? (
              <div className={`border p-5 rounded-3xl flex-1 flex flex-col shadow-sm ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-200'}`}>
                <h3 className={`font-black mb-3 flex items-center gap-2 text-lg ${isDarkMode ? 'text-slate-200' : 'text-slate-800'}`}><Icons.Users c="w-6 h-6" /> รายการกำลังยืม / ออกงาน ({activeOutItems.length})</h3>
          ...
          <div className={`border p-6 rounded-3xl flex flex-col h-full overflow-hidden shadow-sm ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
            <h2 className={`text-xl font-black mb-4 flex items-center gap-2 p-3 rounded-2xl ${isDarkMode ? 'bg-indigo-900/20 text-indigo-400' : 'bg-indigo-50 text-indigo-600'}`}><Icons.ClipboardList c="w-6 h-6"/> ประวัติการเคลื่อนไหว</h2>
    ...
    <div className={`min-h-screen font-sans p-4 sm:p-8 pb-32 transition-colors duration-300 ${theme.mainBg} ${theme.textMain}`}>
      ...
      <div className={`w-full flex flex-col xl:flex-row justify-between items-center mb-8 gap-4 p-6 rounded-2xl shadow-md border ${theme.cardBg}`}>
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 bg-blue-600 text-white rounded-2xl flex items-center justify-center shadow-lg"><Icons.Package c="w-8 h-8" /></div>
          <div>
            <h1 className={`text-2xl sm:text-3xl font-black tracking-tight ${theme.textTitle}`}>MDEC-Stock <span className="text-xs sm:text-sm font-bold bg-blue-100 text-blue-700 px-2 py-1 rounded-lg ml-2 border border-blue-200">v20.8 Long-term</span></h1>
        ...
        <div className="flex flex-wrap justify-center gap-2 sm:gap-3 w-full xl:w-auto">
          <button type="button" onClick={() => setIsDarkMode(!isDarkMode)} className={`p-3 font-bold rounded-xl shadow-sm ${theme.btnCancel}`}>{isDarkMode ? <Icons.Sun c="w-5 h-5" /> : <Icons.Moon c="w-5 h-5" />}</button>

          {isAdmin && (
            <>
              <button type="button" onClick={() => setShowScanModal(true)} className={`px-4 py-3 font-black rounded-xl flex items-center gap-2 shadow-md ${isDarkMode ? 'bg-amber-600 hover:bg-amber-500 text-white' : 'bg-amber-500 hover:bg-amber-600 text-white'}`}><Icons.QrCode c="w-5 h-5" /><span className="hidden sm:inline">สแกน</span></button>
              <button type="button" onClick={() => setShowCommandCenter(true)} className={`px-4 py-3 font-bold rounded-xl flex items-center gap-2 ${isDarkMode ? 'bg-emerald-900/40 text-emerald-400 border border-emerald-800' : 'bg-emerald-50 text-emerald-600'}`}><Icons.Monitor c="w-5 h-5" /><span className="hidden sm:inline">Dashboard</span></button>
              <button type="button" onClick={() => { setBundleForm({ id: null, name: '', itemIds: [] }); setBundleSearchTerm(''); setShowBundleManager(true); }} className={`px-6 py-4 font-black rounded-xl shadow-md text-lg flex items-center gap-2 ${isDarkMode ? 'bg-fuchsia-600 text-white' : 'bg-fuchsia-600 text-white'}`}><Icons.Layers c="w-5 h-5" /> จัดการเซ็ต</button>
              {(settingsOptions.bundles && settingsOptions.bundles.length > 0) && (
                <button type="button" onClick={() => setShowBundleModal(true)} className={`px-6 py-4 font-black rounded-xl shadow-md text-lg flex items-center gap-2 ${isDarkMode ? 'bg-purple-600 text-white' : 'bg-purple-600 text-white'}`}><Icons.Package c="w-5 h-5" /> ใช้งานเซ็ต</button>
              )}
              <button type="button" onClick={() => setShowPersonalItemsModal(true)} className={`px-6 py-4 font-black rounded-xl shadow-md text-lg flex items-center gap-2 ${isDarkMode ? 'bg-pink-600 text-white' : 'bg-pink-600 text-white'}`}><Icons.Tag c="w-5 h-5" /> ของประจำตัว</button>
              <button type="button" onClick={() => setShowQuickReturnModal(true)} className={`px-6 py-4 font-black rounded-xl shadow-md text-lg flex items-center gap-2 ${isDarkMode ? 'bg-indigo-600 text-white' : 'bg-indigo-600 text-white'}`}><Icons.Users c="w-5 h-5" /> ติดตามของรอคืน</button>
              <button type="button" onClick={() => setShowAuditModal(true)} className={`px-4 py-3 font-bold rounded-xl flex items-center gap-2 shadow-sm ${theme.btnCancel}`}><Icons.ClipboardList c="w-5 h-5" /><span className="hidden sm:inline">ดูประวัติ</span></button>
              <button type="button" onClick={() => { setSettingsTab('categories'); setShowSettings(true); }} className={`px-4 py-3 font-bold rounded-xl flex items-center gap-2 shadow-sm ${theme.btnCancel}`}><Icons.Settings c="w-5 h-5" /><span className="hidden sm:inline">ตั้งค่า</span></button>
              <button type="button" onClick={handleLogout} className={`px-4 py-3 font-bold rounded-xl flex items-center gap-2 ${isDarkMode ? 'bg-rose-900/40 text-rose-400 border border-rose-800' : 'bg-rose-50 text-rose-600'}`}><Icons.Unlock c="w-5 h-5" /></button>
            </>
          )}
          {!isAdmin && <button type="button" onClick={() => setShowLogin(true)} className={`px-5 py-3 font-bold rounded-xl flex items-center gap-2 shadow-md ${isDarkMode ? 'bg-blue-600 text-white' : 'bg-slate-800 text-white'}`}><Icons.Lock c="w-5 h-5" /> เข้าสู่ระบบ</button>}
        </div>
      </div>
      ...
      <div className="w-full flex justify-end mb-2 pr-2">
        <button onClick={() => setShowEmptyCategories(!showEmptyCategories)} className={`text-sm font-bold flex items-center gap-1 ${theme.textMuted}`}><Icons.Eye c="w-4 h-4"/> สลับแสดงหมวดหมู่ว่าง</button>
      </div>
      ...
      {isAdmin && selectedItems.length > 0 && (
        <div className={`fixed bottom-8 left-1/2 -translate-x-1/2 backdrop-blur-xl px-4 py-4 sm:px-6 rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.2)] flex items-center gap-4 sm:gap-6 z-40 w-[95%] max-w-4xl justify-between border-2 ${isDarkMode ? 'bg-slate-900/90 border-slate-700 text-white' : 'bg-white/90 border-slate-100 text-slate-800'}`}>
          ...
          <div className="flex gap-2 sm:gap-3 overflow-x-auto custom-scrollbar">
            <button onClick={() => setShowPrintModal(true)} className="px-4 py-3 bg-slate-800 text-white rounded-2xl font-bold shadow-md flex items-center gap-2"><Icons.QrCode c="w-5 h-5"/> พิมพ์ QR</button>
            <button onClick={handleCreateBundleFromSelection} className="px-4 py-3 bg-blue-600 text-white rounded-2xl font-bold shadow-md flex items-center gap-2"><Icons.Layers c="w-5 h-5"/> จัดเซ็ต</button>
            <button onClick={handleOpenBatchBorrow} className="px-4 py-3 bg-purple-600 text-white rounded-2xl font-bold shadow-md flex items-center gap-2"><Icons.UserPlus c="w-5 h-5"/> ยืมออก</button>
            <button onClick={handleOpenBatchEvent} className="px-4 py-3 bg-orange-600 text-white rounded-2xl font-bold shadow-md flex items-center gap-2"><Icons.Truck c="w-5 h-5"/> ออกงาน</button>
            <button onClick={handleOpenBatchReturn} className="px-4 py-3 bg-emerald-600 text-white rounded-2xl font-bold shadow-md flex items-center gap-2"><Icons.CheckCircle c="w-5 h-5"/> รับคืน</button>
            <button onClick={() => setSelectedItems([])} className={`px-4 py-3 rounded-2xl font-bold border shrink-0 ${theme.btnCancel}`}><Icons.X c="w-5 h-5"/></button>
          </div>
        </div>
      )}

      {/* 🔲 หน้าต่างต่างๆ (MODALS) ที่โหลดมาเต็มสูบ */}
      ...
      {showBundleManager && (
        <div className={`fixed inset-0 ${theme.modalOverlay} backdrop-blur-sm flex items-center justify-center p-4 z-[9990]`}>
          <div className={`rounded-3xl shadow-2xl w-full max-w-5xl flex flex-col h-[85vh] overflow-hidden ${theme.cardBg}`}>
            <div className={`flex justify-between items-center p-6 border-b shrink-0 ${theme.divide}`}>
              <h3 className="text-2xl font-black flex items-center gap-3"><Icons.Layers c="w-6 h-6 text-fuchsia-500"/> จัดการเซ็ต</h3>
              <button onClick={()=>setShowBundleManager(false)} className="p-2 hover:text-rose-500"><Icons.X c="w-6 h-6"/></button>
            </div>
            <div className="flex-1 flex flex-col lg:flex-row min-h-0">
              <div className={`w-full lg:w-1/3 flex flex-col border-r ${theme.divide}`}>
                <div className="p-4 font-black text-lg border-b">เซ็ตทั้งหมด ({(settingsOptions.bundles||[]).length})</div>
                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                  {(settingsOptions.bundles||[]).map(b=>(
                    <div key={b.id} className={`p-4 rounded-xl border cursor-pointer ${bundleForm.id===b.id?'border-fuchsia-500 shadow-md':'border-slate-300'}`} onClick={()=>{setBundleForm({id:b.id,name:b.name,itemIds:b.itemIds||[]});setBundleSearchTerm('');}}>
                      <div className="font-bold text-lg">{b.name}</div>
                      <div className="text-sm text-slate-500">{b.itemIds.length} ชิ้น</div>
                      <div className="flex gap-2 mt-2">
                        <button onClick={e=>{e.stopPropagation();setBundleForm({id:b.id,name:b.name,itemIds:b.itemIds||[]});}} className="flex-1 py-1 bg-slate-200 rounded font-bold text-sm text-slate-700">แก้ไข</button>
                        <button onClick={e=>{e.stopPropagation();handleDeleteBundle(b.id);}} className="flex-1 py-1 bg-rose-100 text-rose-600 rounded font-bold text-sm">ลบ</button>
                      </div>
                    </div>
                  ))}
                </div>
              </div>
              <div className="w-full lg:w-2/3 flex flex-col p-6">
                <input type="text" className={`w-full px-4 py-3 mb-4 rounded-xl font-bold border ${theme.input}`} placeholder="ชื่อเซ็ต..." value={bundleForm.name} onChange={e=>setBundleForm({...bundleForm,name:e.target.value})}/>
                <div className={`flex-1 flex flex-col border rounded-xl overflow-hidden ${theme.cardBg}`}>
                  <div className="p-3 border-b"><input type="text" className={`w-full px-3 py-2 rounded-lg border ${theme.input}`} placeholder="ค้นหาอุปกรณ์เข้าเซ็ต..." value={bundleSearchTerm} onChange={e=>setBundleSearchTerm(e.target.value)}/></div>
                ...
                <div className="flex gap-3 mt-4">
                  <button onClick={()=>setBundleForm({id:null,name:'',itemIds:[]})} className={`flex-1 py-3 font-bold rounded-xl ${theme.btnCancel}`}>ล้างค่า</button>
                  <button onClick={handleSaveBundle} className="flex-[2] py-3 bg-fuchsia-600 text-white font-bold rounded-xl">บันทึกเซ็ต</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className={`fixed inset-0 ${theme.modalOverlay} backdrop-blur-sm flex items-center justify-center p-4 z-[9990]`}>
          <div className={`rounded-3xl shadow-2xl w-full max-w-4xl flex flex-col max-h-[85vh] ${theme.cardBg}`}>
            <div className={`flex justify-between items-center p-6 border-b ${theme.divide}`}>
              <h3 className="text-2xl font-black flex items-center gap-3"><Icons.Package c="w-6 h-6 text-purple-500"/> ใช้งานเซ็ตอุปกรณ์</h3>
              <button onClick={()=>setShowBundleModal(false)} className="p-2 hover:text-rose-500"><Icons.X c="w-5 h-5"/></button>
            </div>
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {(settingsOptions.bundles||[]).map(bundle=>{
                ...
                const outCount=(bundle.itemIds||[]).filter(id=>['borrowed','out-for-event'].includes(items.find(i=>i.id===id)?.status)).length;
                return (
                  <div key={bundle.id} className={`p-5 rounded-2xl border flex flex-col lg:flex-row justify-between gap-4 ${theme.cardBg}`}>
                    <div>
                      <h4 className="text-xl font-black">{bundle.name}</h4>
                      <div className="text-sm font-bold text-slate-500 mt-1">พร้อมใช้: {ready}/{bundle.itemIds.length} | รอคืน: {outCount}/{bundle.itemIds.length}</div>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={()=>handleSelectBundleToBorrow(bundle)} disabled={ready===0} className="px-4 py-2 bg-purple-600 text-white rounded-xl font-bold disabled:opacity-50">ยืมเซ็ต</button>
                      <button onClick={()=>handleSelectBundleToEvent(bundle)} disabled={ready===0} className="px-4 py-2 bg-orange-600 text-white rounded-xl font-bold disabled:opacity-50">ออกงาน</button>
                      <button onClick={()=>handleSelectBundleToReturn(bundle)} disabled={outCount===0} className="px-4 py-2 bg-emerald-600 text-white rounded-xl font-bold disabled:opacity-50">รับคืน</button>
                    </div>
                  </div>
                )
              })}
        ...
        <div className={`fixed inset-0 ${theme.modalOverlay} backdrop-blur-sm flex items-center justify-center p-4 z-[9999]`}>
          <div className={`rounded-3xl p-6 sm:p-8 max-w-md w-full max-h-[80vh] flex flex-col shadow-2xl ${theme.cardBg}`}>
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-2xl font-black">ประวัติยืม-คืน</h3>
              <button onClick={()=>setShowHistory(null)}><Icons.X c="w-6 h-6"/></button>
            </div>
            <div className="flex-1 overflow-y-auto space-y-3">
              {(()=>{
                const hList=items.find(i=>i.id===showHistory)?.history||[];
                if(hList.length===0) return <div className="text-center font-bold">ไม่มีประวัติ</div>;
                return hList.slice().reverse().map((h,i)=>(
                  <div key={i} className="p-4 rounded-xl border">
                    <div className="text-sm font-bold mb-1 opacity-70">{new Date(h.date).toLocaleString('th-TH')} - {h.type}</div>
                    <div>{h.type==='return'?`รับคืนโดย: ${h.staffIn}`:h.type==='event'?`งาน: ${h.eventName} (จนท: ${h.staffOut})`:`ผู้ยืม: ${h.borrower} (จนท: ${h.staffOut})`}</div>
                    {h.note && <div className="text-sm italic mt-1">หมายเหตุ: {h.note}</div>}
                  </div>
                ))
              })()}
            </div>
      ...
      {itemToDelete && (
        <div className={`fixed inset-0 ${theme.modalOverlay} backdrop-blur-sm flex items-center justify-center p-4 z-[9999]`}>
          <div className={`rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl ${theme.cardBg}`}>
            <h3 className="text-2xl font-black mb-2">ลบอุปกรณ์?</h3><p className="mb-6">ลบ "{itemToDelete.name}" ออกจากระบบ</p>
            <div className="flex gap-3"><button onClick={()=>setItemToDelete(null)} className={`flex-1 py-3 rounded-xl ${theme.btnCancel}`}>ยกเลิก</button><button onClick={handleDeleteItem} className="flex-1 py-3 bg-rose-600 text-white rounded-xl font-bold">ลบ</button></div>
          </div>
        </div>
      )}
      ...
      {showPersonalItemsModal && (
        <div className={`fixed inset-0 ${theme.modalOverlay} backdrop-blur-sm flex items-center justify-center p-4 z-[9990]`}>
          <div className={`rounded-3xl shadow-2xl w-full max-w-4xl flex flex-col max-h-[85vh] ${theme.cardBg}`}>
            <div className="flex justify-between p-6 border-b"><h3 className="text-2xl font-black flex gap-2"><Icons.Tag c="w-6 h-6 text-pink-500"/> ของประจำตัวเจ้าหน้าที่</h3><button onClick={()=>setShowPersonalItemsModal(false)}><Icons.X c="w-5 h-5"/></button></div>
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {(()=>{
                const groups={};
                items.forEach(i=>{if(i.owner){groups[i.owner]=groups[i.owner]||[];groups[i.owner].push(i);}});
                const keys=Object.keys(groups).sort();
                return keys.length===0?<div className="text-center font-bold py-10">ไม่มีของประจำตัว</div>:keys.map(o=>(
                  <div key={o} className="p-5 border rounded-2xl">
                    <h4 className="font-black text-xl mb-2">{o} ({groups[o].length} ชิ้น)</h4>
                    <div className="space-y-1">{groups[o].map(i=><div key={i.id} className="text-sm border-b pb-1">- {i.name} ({i.sn})</div>)}</div>
                  </div>
                ))
              })()}
            </div>
      ...
      {showQuickReturnModal && (
        <div className={`fixed inset-0 ${theme.modalOverlay} backdrop-blur-sm flex items-center justify-center p-4 z-[9990]`}>
          <div className={`rounded-3xl shadow-2xl w-full max-w-4xl flex flex-col max-h-[85vh] ${theme.cardBg}`}>
            <div className="flex justify-between p-6 border-b"><h3 className="text-2xl font-black flex gap-2"><Icons.Users c="w-6 h-6 text-indigo-500"/> ติดตามของรอคืน</h3><button onClick={()=>setShowQuickReturnModal(false)}><Icons.X c="w-5 h-5"/></button></div>
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {activeGroups.length===0?<div className="text-center font-bold py-10">ไม่มีของรอคืน</div>:activeGroups.map((g,i)=>(
                <div key={i} className="p-5 border rounded-2xl flex justify-between items-center">
                  <div><h4 className="font-black text-xl">{g.type==='event'?'งาน: ':'ผู้ยืม: '}{g.name}</h4><div className="text-sm">{g.ids.length} ชิ้น</div></div>
                  <button onClick={()=>{setReturnTargetIds([...g.ids]);setReturnChecklist([]);setReturnData({staff:'',newStaff:''});setShowQuickReturnModal(false);}} className="px-6 py-3 bg-emerald-600 text-white rounded-xl font-bold">รับคืนกลุ่มนี้</button>
                </div>
              ))}
            </div>
          </div>
      ...
      {showAuditModal && (
        <div className={`fixed inset-0 ${theme.modalOverlay} backdrop-blur-sm flex items-center justify-center p-4 z-[9990]`}>
          <div className={`rounded-3xl shadow-2xl w-full max-w-3xl flex flex-col max-h-[85vh] ${theme.cardBg}`}>
            <div className="flex justify-between p-6 border-b"><h3 className="text-2xl font-black flex gap-2"><Icons.ClipboardList c="w-6 h-6 text-blue-500"/> ประวัติส่วนกลาง</h3><button onClick={()=>setShowAuditModal(false)}><Icons.X c="w-5 h-5"/></button></div>
            <div className="flex-1 overflow-y-auto p-6 space-y-3">
              {auditLogs.length===0?<div className="text-center font-bold py-10">ไม่มีประวัติ</div>:auditLogs.map(l=>(
                <div key={l.id} className="p-4 border rounded-xl">
                  <div className="text-sm font-bold opacity-70">{new Date(l.timestamp).toLocaleString('th-TH')} - {l.action}</div>
                  <div className="font-bold text-lg">{l.target}</div><div className="text-sm mt-1">{l.details}</div>
                </div>
              ))}
              {auditLogs.length >= auditLimit && (
                <button onClick={() => setAuditLimit(p => p + 50)} className={`w-full py-3 mt-4 font-bold rounded-xl border ${theme.btnCancel}`}>โหลดเพิ่ม...</button>
              )}
            </div>
          </div>
        </div>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {settingsTab === 'database' ? (
                <div className="space-y-4">
                  <button onClick={exportToCSV} className="w-full py-3 bg-emerald-600 text-white font-bold rounded-xl flex justify-center items-center gap-2"><Icons.Download c="w-5 h-5"/> โหลดไฟล์ CSV (Backup)</button>
                  <div className="border-t pt-4 mt-4">
                    <input type="file" accept=".csv" className="hidden" ref={fileInputRef} onChange={handleImportCSV} />
                    <button onClick={() => fileInputRef.current?.click()} className="w-full py-3 bg-blue-600 text-white font-bold rounded-xl flex justify-center items-center gap-2"><Icons.Upload c="w-5 h-5"/> นำเข้าข้อมูล (Import CSV)</button>
                  </div>
                </div>
              ) : (
                <>
                  ...
                  <div className="space-y-2">
                    {(settingsOptions[settingsTab] || []).filter(c => c !== 'อื่นๆ').map((item, index) => (
                      <div key={index} className={`flex justify-between items-center p-3 border rounded-xl ${theme.btnSecondary}`}>
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => { setEditingSettingItem(item); setNewSettingItem(item); }} className="p-2 text-blue-500 hover:bg-blue-100 rounded-lg"><Icons.Edit c="w-4 h-4"/></button>
                          <button onClick={() => setDeleteSettingConfirm(item)} className="p-2 text-rose-500 hover:bg-rose-100 rounded-lg"><Icons.Trash c="w-4 h-4"/></button>
                        </div>
                      </div>
                    ))}
                  </div>
